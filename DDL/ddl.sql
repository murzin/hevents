...
CREATE TABLE IF NOT EXISTS events (
    ...
    FOREIGN KEY (etp_id) REFERENCES event_types(etp_id),
    FOREIGN KEY (epl_id) REFERENCES event_places(epl_id),
    FOREIGN KEY (eer_id) REFERENCES event_era(eel_id)
);
...
CREATE TABLE IF NOT EXISTS event_links (
    ...
    FOREIGN KEY (evt_id_1) REFERENCES events(evt_id),
    FOREIGN KEY (evt_id_2) REFERENCES events(evt_id)
);

CREATE TABLE IF NOT EXISTS event_era (
    eer_id INTEGER PRIMARY KEY,
    eer_name TEXT NOT NULL
);
